            ...
            <div class="space-y-1">
              ...
              <div class="relative">
                <input 
                  ...
                  :class="[
                    'w-full px-2.5 py-2 sm:py-2.5 pr-10 text-xs border rounded-lg transition-all duration-300 bg-white/60 backdrop-blur-sm placeholder-gray-400 hover:bg-white/80 focus:bg-white/90 touch-manipulation min-h-[40px]',
                    errors.password 
                      ? 'border-red-300 focus:border-red-500 focus:ring-red-100' 
                      : 'border-gray-200/60 focus:border-red-500 focus:ring-red-100 hover:border-red-300 hover:shadow-lg'
                  ]"
                ...
                />
                <!-- Updated icon size to match email icon (h-3.5 w-3.5) -->
                <button 
                  type="button"
                  @click="showPassword = !showPassword"
                  class="absolute right-3 top-1/2 flex items-center justify-center text-gray-400 static-button password-toggle"
                  :disabled="isLoading"
                >
...
@media (max-width: 768px) {
  button:not(.password-toggle), input:not([type="checkbox"]), [role="button"] {
    min-height: 44px;
    min-width: 44px;
  }
}
...
.static-button {
  ...
  transform: translateY(-50%) !important; /* Fixed vertical alignment */
...
.static-button:visited {
  ...
  outline: none !important;
  box-shadow: none !important;
  transform: translateY(-50%) !important; /* Ensure it stays centered on hover */
  transition: none !important;
  color: #6b7280 !important; /* darker gray on hover for better feedback */
}
