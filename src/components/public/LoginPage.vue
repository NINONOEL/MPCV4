<template>
  <div class="min-h-screen bg-gradient-to-br from-red-50 via-slate-50 to-orange-50/30 flex items-center justify-center p-2 sm:p-4 lg:p-6 font-sans relative overflow-hidden">
    ...
    <div class="relative w-full max-w-xs sm:max-w-sm lg:max-w-md">
      ...
      <div class="bg-white/80 backdrop-blur-2xl rounded-xl sm:rounded-2xl shadow-2xl border border-white/60 overflow-hidden hover:shadow-3xl transition-all duration-500 hover:bg-white/85">
        <!-- Enhanced responsive header -->
        <div class="px-4 sm:px-6 py-4 sm:py-6 text-center border-b border-red-50/50 bg-gradient-to-r from-red-50/30 to-orange-50/30">
          <!-- Responsive typography with better scaling -->
          <div class="inline-flex items-center justify-center w-10 sm:w-12 h-10 sm:h-12 bg-gradient-to-br from-red-600 to-orange-500 rounded-xl sm:rounded-2xl shadow-xl mb-3 sm:mb-4 hover:scale-105 transition-transform duration-300 touch-manipulation">
          ...
          <h1 class="text-lg sm:text-xl lg:text-2xl font-bold bg-gradient-to-r from-red-800 to-orange-600 bg-clip-text text-transparent mb-2 leading-tight">Welcome Back to Your Creative Space</h1>
          <p class="text-xs text-gray-600 leading-relaxed px-2">Log in to explore your palette of options and premium paint solutions.</p>
        </div>

        <!-- Enhanced responsive form body -->
        <div class="px-4 sm:px-6 py-4 sm:py-6">
          <!-- Enhanced responsive error display -->
          <div v-if="errorMessage" class="mb-3 sm:mb-4 p-3 bg-red-50/80 backdrop-blur-sm border border-red-100/60 rounded-xl">
          ...
          <div class="space-y-2 sm:space-y-3 mb-4 sm:mb-6">
            ...
            <button 
              ...
              :disabled="isLoading"
              class="w-full flex items-center justify-center px-3 sm:px-4 py-2.5 sm:py-3 border border-gray-200/60 rounded-xl text-gray-700 bg-white/60 backdrop-blur-sm hover:bg-white/80 hover:border-red-200 transition-all duration-300 text-xs sm:text-sm font-semibold disabled:opacity-50 hover:shadow-xl hover:-translate-y-1 group touch-manipulation min-h-[44px]"
            >
              <svg class="w-4 h-4 mr-2 group-hover:scale-110 transition-transform duration-300 flex-shrink-0" viewBox="0 0 24 24">
          ...
          <div class="relative my-4 sm:my-6">
            ...
            <div class="relative flex justify-center text-xs">
              <span class="px-3 sm:px-4 bg-white/80 backdrop-blur-sm text-gray-500 font-medium rounded-full">or sign in with email</span>
            </div>
          </div>
          ...
          <form @submit.prevent="handleLogin" class="space-y-3 sm:space-y-4">
            ...
            <div class="space-y-1.5">
              ...
              <div class="relative">
                <input 
                  ...
                  v-model="form.email"
                  :class="[
                    'w-full px-3 py-2.5 sm:py-3 text-sm border rounded-xl transition-all duration-300 bg-white/60 backdrop-blur-sm placeholder-gray-400 hover:bg-white/80 focus:bg-white/90 touch-manipulation min-h-[44px]',
                    errors.email 
            ...
            <div class="space-y-1.5">
              ...
              <div class="relative">
                <input 
                  ...
                  :class="[
                    'w-full px-3 py-2.5 sm:py-3 pr-10 text-sm border rounded-xl transition-all duration-300 bg-white/60 backdrop-blur-sm placeholder-gray-400 hover:bg-white/80 focus:bg-white/90 touch-manipulation min-h-[44px]',
                    errors.password 
                      ? 'border-red-300 focus:border-red-500 focus:ring-red-100' 
                      : 'border-gray-200/60 focus:border-red-500 focus:ring-red-100 hover:border-red-300 hover:shadow-lg'
                  ]"
                ...
                />
                <!-- Fixed password icon centering to match email icon exactly -->
                <button 
                  type="button"
                  @click="showPassword = !showPassword"
                  class="absolute right-3 top-1/2 -translate-y-1/2 flex items-center justify-center text-gray-400 static-button"
                  :disabled="isLoading"
                >
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between text-xs pt-1 space-y-2 sm:space-y-0">
              <label class="flex items-center group cursor-pointer touch-manipulation">
                <input 
                  type="checkbox" 
                  v-model="form.rememberMe" 
                  class="rounded border-gray-300 text-red-600 focus:ring-red-500 w-3.5 h-3.5 transition-colors duration-300"
                  :disabled="isLoading"
                >
                <span class="ml-2 text-gray-600 group-hover:text-gray-800 transition-colors duration-300">Remember me</span>
              </label>
              <button 
                type="button" 
                class="text-red-600 hover:text-orange-600 font-semibold transition-colors duration-300 hover:underline text-left sm:text-right touch-manipulation min-h-[40px] flex items-center"
                @click="handleForgotPassword"
                :disabled="isLoading || forgotPasswordLoading"
              >
                {{ forgotPasswordLoading ? 'Sending...' : 'Need help? Reset your password.' }}
              </button>
            </div>
            ...
            <button 
              type="submit"
              :disabled="isLoading"
              class="w-full bg-gradient-to-r from-red-600 to-orange-500 hover:from-red-700 hover:to-orange-600 text-white font-bold py-2.5 sm:py-3 px-4 rounded-xl transition-all duration-300 disabled:opacity-50 hover:shadow-2xl hover:-translate-y-1 text-sm group relative overflow-hidden touch-manipulation min-h-[44px]"
            >
              ...
              <template v-else>
                <div class="inline-flex items-center justify-center relative z-10">
                  <svg class="h-4 w-4 mr-2 group-hover:scale-105 transition-transform duration-300 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1"></path>
                  </svg>
                  <span class="truncate">Enter Your Portal</span>
                </div>
              </template>
            </button>
          </form>
          ...
          <div class="text-center mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-gray-100/60">
